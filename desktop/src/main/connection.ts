// Desktop-owned connection model: error classification, the panel connection
// state, and the health-badge view-model.
//
// WHY THIS IS NOT IN client/ : everything under src/main/client/ is copied
// verbatim from the VS Code extension and is frozen (DECISIONS.md §3). The
// extension's `isConnectionError` / `computeConnectionState` collapse "the
// server is not running" and "the server rejected your token" into one
// `unreachable` state. Verified against the live Engraphy server (0.1.0): the MCP
// SDK surfaces a 401 as the string
//   "Streamable HTTP error: Error POSTing to endpoint: unauthorized"
// which carries no status digits, matches the copied predicate's /Unauthorized/i
// branch, and therefore makes a HEALTHY server render "No server connected,
// start one first". That is the wrong remedy for the most common real failure.
// This module owns the three-way split the desktop app needs. See DECISIONS.md
// §12 for what should be pushed back upstream.
//
// Pure: no electron, no SDK, no DOM imports, so scripts/test-client.js covers it.
...
export type ConnectionKind = 'ok' | 'unconfigured' | 'unreachable' | 'unauthorized';

/**
 * Desktop superset of the extension's ConnectionVM. `kind:'ok'` renders the
 * normal bands; every other kind renders the recovery block, with remedy copy
 * chosen by kind.
 */
export interface DesktopConnectionVM {
	kind: ConnectionKind;
	summary?: string;
	detail?: string;
	code?: string | null;
 ...
 *
 * Preserves the extension's rule that ONE band failing is a per-band problem
 * rendered inline, not a dead server: every slot must be non-null before the
 * panel is swapped for the recovery block. Adds the three-way split: all-auth
 * failures mean `unauthorized` (server is up, token is wrong), all-transport
...
export function computeConnection(opts: {
	...
	if (described.every((d) => d.class === 'transport' || d.class === 'config')) {
		return { kind: 'unreachable', summary: first.summary, detail: first.detail, code: first.code };
	}
	return { kind: 'ok' };
}
...
export interface HealthVM {
	phase: HealthPhase;
	/** Badge text. Short enough for the sidebar. */
	label: string;
	/** Tooltip. */
	title: string;
	/** Raw error detail for the disconnected banner's disclosure. */
	detail?: string;
	/** True when the app can actually read data. Drives the global banner. */
	usable: boolean;
	/**
	 * One-line banner copy for the not-usable states. Built here rather than in
	 * the renderer so the badge, the banner and the panel recovery block cannot
	 * disagree with each other about what is wrong.
	 */
	banner?: string;
	space?: string;
	version?: string;
...
/**
 * Build the sidebar badge state.
 *
 * The load-bearing rule: /healthz is UNAUTHENTICATED on Engraphy, so it answers
 * 200 for a server you hold no valid token for. Reporting "Connected" off
 * /healthz alone is exactly how you get a green badge sitting over four erroring
 * panels (the shipped behaviour before this change, reproduced against the live
 * server with no token set). The badge goes green only when the AUTHENTICATED
 * probe succeeded.
 */
export function buildHealthVM(p: HealthProbe): HealthVM {
	...
	if (!p.configured) {
		return {
			phase: 'unconfigured',
			label: 'No server set',
			banner: 'No server connected yet. Point Engraphy at your Engraphy server to get started.',
			title: 'Engraphy is not pointed at a memory server yet. Click to open Settings.',
			usable: false,
		};
	}
	if (p.authOk) {
		...
		return {
			phase: 'connected',
			label: p.space ? 'Connected · ' + p.space : 'Connected',
			title: bits.join(', ') + '. Click to re-check.',
			usable: true,
	...
	if (d.class === 'auth') {
		return {
			phase: 'unauthorized',
			label: p.hasToken ? 'Token rejected' : 'Token needed',
			banner: p.hasToken
				? 'Your token was rejected. The server is running, but it will not let this app read anything.'
				: host + ' is running, but it needs a token before it will return anything.',
			title: p.hasToken
				? host +
					' is running but rejected your token. Mint a fresh one with "engraphy-admin token create" and paste it into Settings.'
				: host + ' is running but requires a token. Add one in Settings.',
			detail: d.detail,
			usable: false,
		};
	}
	...
	return {
		phase: 'degraded',
		label: 'Server error',
		banner: host + ' answered, but the connection check failed: ' + d.summary,
		title: host + ' answered but the check failed: ' + d.summary,
		detail: d.detail,
		usable: false,
	};
}
