...
- **Nodes** authored with `source_session = S` and `status IN ('active','superseded')` → `status = 'archived'` — out of search, briefing, traversal, and dedup candidacy, closed to `update` and `link` (`ENGRAPHY_VALIDATION`), still readable by id, reversible by hand. (Rows already `merged` keep their status: the merge-chain CHECK ties `merged` to `canonical_id`, reads already resolve them away to canonical, and their absorbed content is handled as addenda, next.)
