...
**Decision: RLS enabled on all data tables. Two policy layers: space pinning (`space_id = current_setting('engram.space_id')`) and, on nodes/edges, readable-scope filtering via `engram_readable_scopes()` reading both GUCs (`engram.space_id`, `engram.principal`) — the same function the application queries use, so app and backstop cannot disagree ([06](06-teams-and-sharing.md)). The app's DB role is *not* `BYPASSRLS`.**
...
| RLS probe | Session with `engram.space_id = 'alex'` sees zero `nova` rows through every query shape, including a deliberately filter-free `SELECT`; within a team space, a member-pinned session sees zero rows from teammates' private scopes the same way |
