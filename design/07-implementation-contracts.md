...
**`supersede`:** returns the inserted node's `write` envelope plus `"superseded": "<old_id>"` (the analogue of `resolve_duplicate`'s rule — a write whose outcome names its side effect). A replacement typed differently from the old node, or one that bands MERGE or PENDING against a third node, **downgrades to a plain write**: the call returns that band's `write` envelope plus `supersede_downgraded` (`reason`, `old_id`, `old_kept_active: true`, and `related_edge_added` for a cross-type replacement or `band_outcome` for a band collision), writes no `supersedes` edge, flips no status, and leaves the old node `active`. A cross-type replacement gets a `relates_to` edge to the old node when the pack declares that rule.
...
**Admin tools** (all five space_admin-gated → `ENGRAPHY_ROLE` otherwise; absent from `tools/list` entirely when config `space_admin_tools: false`; every call audited). **`admin_token_create`** → `{"v": 1, "token": "<plaintext>", "principal": "…", "client_name": "…", "role": "…", "created_at": "…"}` — the plaintext exists in exactly one place ever: this envelope. The server stores only the SHA-256; the audit row records mint metadata, never the token or its hash; there is no retrieve-later path (lost = revoke + re-mint). **`admin_member_add`** → `{"v": 1, "principal": {…created row…}}` (creates the principal only — no personal scope; see [06](06-teams-and-sharing.md)). **`admin_member_archive`** → `{"v": 1, "principal": {…updated row…}}`: `archived` absent or `true` archives the member (every token it holds is refused at the door from its next request), `false` restores it; a space_admin archiving itself ⇒ `ENGRAPHY_VALIDATION`; unknown member ⇒ `ENGRAPHY_NOT_FOUND`. Minting a token for an archived principal ⇒ `ENGRAPHY_VALIDATION`. **`admin_scope_visibility`** → `{"v": 1, "scope": {…updated row, scope_list's shape…}}`. **`admin_grant`** → `{"v": 1, "grant": {"scope_id", "principal", "level"}}`.
...
| `admin_member_archive` | `id` | string | yes | |
| | `archived` | boolean | no | `true`; `false` restores an archived member |
