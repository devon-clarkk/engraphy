...
CREATE FUNCTION engraphy_readable_scopes() RETURNS SETOF text
...
CREATE FUNCTION engraphy_writable_scopes() RETURNS SETOF text
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = engraphy AS $$
  SELECT s.id FROM scopes s
  WHERE s.space_id = current_setting('engraphy.space_id', true)
    AND s.archived = false
...
async with pool.connection() as conn:
    async with conn.transaction():
        await conn.execute("SELECT set_config('engraphy.space_id', %s, true), "
                           "set_config('engraphy.principal', %s, true)",
...
CREATE POLICY nodes_read ON nodes FOR SELECT
  USING (space_id = current_setting('engraphy.space_id', true)
         AND scope_id IN (SELECT engraphy_readable_scopes()));

CREATE POLICY nodes_write ON nodes FOR INSERT
  WITH CHECK (space_id = current_setting('engraphy.space_id', true)
              AND scope_id IN (SELECT engraphy_writable_scopes()));
