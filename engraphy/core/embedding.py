...
trade are measured rather than estimated. Against `onnx-int8`, one profile per
fresh process on one Linux x86-64 host, and fused recall@10 over 498
evidence-bearing LoCoMo questions against the same 1297 turn-nodes:

    steady RSS     143MB against 262MB
    ms per embed   5.1 against 23.1
    recall@10      0.643 against 0.688,  about -6% relative

Both recall figures are the spread of repeated runs rather than one number:
micro measured 0.6446 and 0.6426 across two independent loads of the same
corpus, int8 0.6847 and 0.6908. The variation is the approximate HNSW index,
not the embedder, and it is roughly three questions out of 498 either way.

Every absolute cosine this engine compares against is calibrated separately for
it, because those numbers sit on a scale the MODEL owns rather than one this
engine chose: the dedup bands in `core/dedup.py`, `resonance.floor` beside them,
`briefing.semantic_floor` in `core/briefing.py`, and read-time near-duplicate
collapse in `core/search.py`. gte-small scores a clearly-unrelated pair around
0.71 where nomic scores it around 0.45, so a floor carried over unchanged would
sit under the entire store and nothing would error.
