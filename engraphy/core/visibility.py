...
engraphy_readable_scopes()/engraphy_writable_scopes() in the database are the single
...
wrapper sets engraphy.space_id / engraphy.principal before this ever runs) — this
...
def readable_scopes(cursor) -> set[str]:
    ...
    cursor.execute("SELECT engraphy_readable_scopes()")
    return {row[0] for row in cursor.fetchall()}


def writable_scopes(cursor) -> set[str]:
    """The current session identity's writable scope ids."""
    cursor.execute("SELECT engraphy_writable_scopes()")
    return {row[0] for row in cursor.fetchall()}
...
def ambient_scope_set(cursor, explicit_scopes: set[str]) -> set[str]:
    ...
    cursor.execute(
        "SELECT id FROM scopes WHERE space_id = current_setting('engraphy.space_id', true) "
        "AND ambient = true"
    )
