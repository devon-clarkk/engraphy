...
#   2. provision-- create the non-superuser engram_app role and its GRANTs
...
# the far-less-privileged engram_app role this script provisions.
set -eu

echo "[init] applying migrations (in-process, no dbmate required)..."
engraphy-admin migrate --dump-dir /backups

echo "[init] provisioning engram_app role and grants..."
